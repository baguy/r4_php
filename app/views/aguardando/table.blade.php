@if ($elements->count())
  ...
  @define $isAdmin = Auth::user()->hasRole('GERENTE')
      ...
      <thead>
        ...
        @if(Auth::user()->hasRole('GERENTE'))
          <div class="form-group col-md-12 text-right">
            ...
            <a href="#modalAprovar" class="btn btn-info"
             data-toggle="modal"
             data-target="#modalAprovar" onclick="selecionarParaAprovar()">{{ trans('exames.aprovar') }}</a>
            ...
            @if (!$C_group)
              ...
              @else
                ...
                @include('templates/parts/datatable/_datatable__options--buttons', [
                  ...
                  'route_show'      => route('solicitacoes.show', [ $exame->solicitacao_id ]),
                  'route_restore'   => route('exames.restore', [ $exame->id ]),
                  'is_printable'    => false
                ])
  ...
  @if ($isAdmin)
    ...
    @foreach ($elements as $exame)
      ...
      @if(!$isTrashed)
        ...
        @include('aguardando/_modal-aprovar')

      @endif

      @if($isTrashed)

        @include('exames/_modal-restore')
