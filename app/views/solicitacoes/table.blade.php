  ...
  <div class="table-responsive">
    ...
    <table class="table table-hover table-sm table-collapse datatable">
      ...
      <tbody>
        ...
        @foreach ($elements as $solicitacao)
          ...
          <tr class="{{ ($isTrashed) ? 'table-danger' : '' }}">
            ...
            @if (!$C_group)
              ...
              @include('templates/parts/datatable/_datatable__collapse--one', [
                'collapse_id' => "collapseUnidade_$solicitacao->id"
              ])

              <td class="align-middle w-auto">
                <span class="ellipsis">{{ isset($solicitacao->paciente->nome)?$solicitacao->paciente->nome:null }}</span>
              </td>

              <td class="align-middle w-auto">
                <span class="ellipsis">{{ isset($solicitacao->numero)?$solicitacao->numero:null }}</span>
              </td>
            ...
            @if ($C_group)
              ...
              @include('templates/parts/datatable/_datatable__total--td', [ 'model' => $solicitacao ])
            ...
            @if (!$C_group)
              ...
              <td class="align-middle col-options-3">
                ...
                @include('templates/parts/datatable/_datatable__options--buttons', [
                  'is_trashed'      => $isTrashed,
                  'has_permission' => Auth::user()->hasRole('USER'),
                  'modal_id'        => $solicitacao->id,
                  'route_edit'      => route('solicitacoes.edit', [ $solicitacao->id ]),
                  'route_show'      => route('solicitacoes.show', [ $solicitacao->id ]),
                  'is_printable'    => false
                ])
          ...
          <tr>
            ...
            <td class="description">
              ...
              <div class="collapse" id="collapseUnidade_{{ $solicitacao->id }}">
                  ...
                  <div class="row">
                    ...
                    <div class="col-12">
                      ...
                      <blockquote class="blockquote d-block">
                        <p class="mb-0">
                          <i class="fas fa-id-card fa-fw mr-1"></i> {{ trans('pacientes.lbl.sus') }}
                        </p>
                        <footer class="blockquote-footer">
                          {{ $solicitacao->paciente->sus }}
                        </footer>
                      </blockquote>
